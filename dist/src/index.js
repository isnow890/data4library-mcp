...
function toolFn(name, fn) {
    return async (data, ctx) => {
        ...
        try {
            ...
            const duration = Date.now() - ts;
            console.error(`[${name}] failed after %dms: %s`, duration, error instanceof Error ? error.message : String(error));
            throw error;
        }
    };
}
...
server.addTool({
    ...
    execute: toolFn("session_stats", async () => {
        ...
        const sessionDuration = Math.round((Date.now() - debugStats.startTime) / 1000);
