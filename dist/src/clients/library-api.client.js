...
export class LibraryApiClient {
     ...
     * 인기대출도서 조회
     */
    async searchPopularBooks(params = {}) {
     ...
     * 마니아를 위한 추천도서 조회
     */
    async getManiaRecommendations(isbn13) {
     ...
     * 다독자를 위한 추천도서 조회
     */
    async getReaderRecommendations(isbn13) {
        const url = `${this.baseUrl}/api/recommandList`;
     ...
     * 도서 키워드 목록
     */
    async getBookKeywords(isbn13, additionalYN = "N") {
     ...
     * 도서별 이용 분석
     */
    async getBookUsageAnalysis(isbn13) {
     ...
     * 도서관/지역별 인기대출 도서 조회
     */
    async searchPopularBooksByLibrary(params) {
     ...
     */
    async checkBookAvailability(params) {
     ...
     * 도서관별 인기대출도서 통합
     */
    async getPopularBooksByLibrary(libCode) {
     ...
     * 이달의 키워드
     */
    async getMonthlyKeywords(params = {}) {
     ...
     * 지역별 독서량/독서율
     */
    async getReadingQuantity(params = {}) {
     ...
     * 신착도서 조회
     */
    async getNewArrivalBooks(params) {
     ...
     * 도서관 코드 검색 (로컬 데이터 기반)
     */
    async searchLibraryCodes(params) {
        const { libraryName, regionName, limit = 20 } = params;
        let filteredLibraries = [...this.libraries];
        // 도서관명으로 검색
        if (libraryName) {
            const searchTerm = libraryName.toLowerCase();
            filteredLibraries = filteredLibraries.filter(lib => lib.libName?.toLowerCase().includes(searchTerm) ||
                lib.libCode?.toLowerCase().includes(searchTerm));
        }
        // 지역명으로 검색
        if (regionName) {
            const searchTerm = regionName.toLowerCase();
            filteredLibraries = filteredLibraries.filter(lib => lib.address?.toLowerCase().includes(searchTerm) ||
            ...
            content: [
                {
                    ...
                    text: `검색 결과: ${filteredLibraries.length}개 도서관 중 ${searchResults.length}개 표시\n\n` +
                        searchResults.map(lib => `📚 ${lib.libName} (${lib.libCode})\n` +
                            `📍 주소: ${lib.address}\n` +
                            `📞 전화: ${lib.tel || '전화번호 없음'}\n` +
     ...
     * 세부지역 코드 조회 (상수 기반)
     */
    async getDetailedRegionCodes(params) {
        const regionName = REGION_CODES[params.region];
        if (!regionName) {
     ...
     * 세부주제 코드 조회 (상수 기반)
     */
    async getDetailedSubjectCodes(params) {
        const subjectInfo = KDC_SUBJECT_CODES[params.kdc];
        if (!subjectInfo) {
     ...
     */
    async searchDetailedKDCCodes(params) {
        ...
        if (keyword) {
            ...
            filteredCodes = filteredCodes.filter(([, name]) => name.toLowerCase().includes(searchTerm));
        }
        // 결과 제한
        ...
        const searchResults = results.map(([code, name]) => ({
            code,
            name,
            majorCategory: KDC_SUBJECT_CODES[code[0]]?.name || '알수없음'
        }));
        return {
     ...
     */
    async searchDetailedRegionCodes(params) {
        ...
        if (keyword) {
            const searchTerm = keyword.toLowerCase();
            filteredCodes = filteredCodes.filter(([, regionInfo]) => regionInfo.province.toLowerCase().includes(searchTerm) ||
